...
import { canReadDelivery, deliveryScope, getDeliveryRun } from "@/modules/delivery-settlement";
...
export async function GET(_request: Request, context: { params: Promise<{ runId: string }> }) {
  ...
  const run = await getDeliveryRun(scope, (await context.params).runId);
  return run ? NextResponse.json({ data: run }) : NextResponse.json({ error: { code: "NOT_FOUND", message: "Sync run tidak ditemukan." } }, { status: 404 });
}
