...
import { canReadDelivery, deliveryScope, deliverySettlementListSchema, listDeliverySettlements } from "@/modules/delivery-settlement";
...
export async function GET(request: Request) {
  ...
  const parsed = deliverySettlementListSchema.safeParse(Object.fromEntries(new URL(request.url).searchParams));
  if (!parsed.success) return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: "Filter tidak valid." } }, { status: 400 });
  return NextResponse.json(await listDeliverySettlements({ ...scope, ...parsed.data }));
}
