...
export async function GET(request: Request) {
  ...
  if (!canReadProblemWaybill(session)) return NextResponse.json({ error: { code: "FORBIDDEN", message: "Akses tidak diizinkan." } }, { status: 403 });
  ...
  const parsed = problemWaybillQuerySchema.safeParse(Object.fromEntries(new URL(request.url).searchParams));
  if (!parsed.success) return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: "Filter tidak valid." } }, { status: 400 });
  return NextResponse.json(await listProblemWaybillDelivery({ ...scope, ...parsed.data }));
}
