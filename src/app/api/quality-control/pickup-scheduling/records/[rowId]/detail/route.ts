...
export async function GET(request: Request, context: { params: Promise<{ rowId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await getPickupSchedulingDetail({
      ...scope, actorId: session.userId, startDate: parsed.data.startDate, endDate: parsed.data.endDate,
      rowId, sessionOutletCode: session.outletCode,
      requestId: request.headers.get("x-request-id") || randomUUID(),
    }), { headers: noStore });
  } catch (error) {
    const code = error && typeof error === "object" && "code" in error ? error.code : null;
    return NextResponse.json(
      { error: { code: code === "NOT_FOUND" ? "ROW_NOT_FOUND" : "DETAIL_UNAVAILABLE" } },
