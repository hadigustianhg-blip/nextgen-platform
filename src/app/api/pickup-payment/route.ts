...
import {
  canCreatePickupPayment, canReadPickupPayment, createPickupPaymentWithProof, listPickupPayment,
  pickupPaymentInputSchema, pickupPaymentListSchema, pickupPaymentScope,
} from "@/modules/payment";
import { firstValidationMessage, isSameOriginRequest, pickupPaymentErrorStatus, pickupPaymentRequestPayload } from "@/modules/payment/pickup-payment.http";
...
export async function POST(request: Request) {
  ...
  const parsed = pickupPaymentInputSchema.safeParse(payload.values);
  if (!parsed.success) return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: firstValidationMessage(parsed.error) } }, { status: 400 });
  try {
    const payment = await createPickupPaymentWithProof({ ...scope, actorId: session.userId }, parsed.data, payload.proof);
    ...
    const message = code === "PICKUP_PAYMENT_NOT_CASH_SETTLEMENT"
      ...
      : undefined;
    return NextResponse.json({ error: { code, message } }, { status: pickupPaymentErrorStatus(code) });
  }
}
