...
import { canManageProfitLoss, profitLossEntrySchema, profitLossScope, profitLossVoidSchema, updateManualEntry, voidManualEntry } from "@/modules/profit-loss";
...
export async function PATCH(request: Request, context: Context) {
  ...
  return NextResponse.json({ data: await updateManualEntry(access.context!, (await context.params).id, parsed.data) });
}
export async function DELETE(request: Request, context: Context) {
  const access = await auth(); if (access.response) return access.response;
  const parsed = profitLossVoidSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: { code: "VALIDATION_ERROR" } }, { status: 400 });
  return NextResponse.json({ data: await voidManualEntry(access.context!, (await context.params).id, parsed.data.reason) });
}
