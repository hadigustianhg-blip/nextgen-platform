...
import {
  ...
  salaryErrorResponse,
  salaryKasbonAllocationUpdateSchema,
  salaryScope,
  updateSalaryKasbonAllocation,
} from "@/modules/salary";
...
export async function PATCH(request: Request, context: Context) {
  ...
  const parsed = salaryKasbonAllocationUpdateSchema.safeParse(
  ...
  try {
    return NextResponse.json({ data: await updateSalaryKasbonAllocation({
      ...scope,
      actorId: session.userId,
      outletCode: session.outletCode,
    }, (await context.params).id, parsed.data.amount) });
  } catch (error) {
