...
import {
  canCreateManualCashFlow, cashFlowScope, createManualExpense, manualExpenseSchema,
...
export async function POST(request: Request) {
  ...
  const parsed = manualExpenseSchema.safeParse(await request.json());
  if (!parsed.success) return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: parsed.error.issues[0]?.message } }, { status: 400 });
  return NextResponse.json({ data: await createManualExpense({ ...scope, actorId: session.userId }, parsed.data) }, { status: 201 });
}
