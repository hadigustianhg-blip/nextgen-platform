...
import {
  canManageManualCashFlow, cashFlowScope, voidCashMovementSchema, voidManualMovement,
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parsed = voidCashMovementSchema.safeParse(await request.json());
  if (!parsed.success) return NextResponse.json({ error: { code: "VALIDATION_ERROR" } }, { status: 400 });
  try {
    const movement = await voidManualMovement({ ...scope, actorId: session.userId }, (await context.params).id, parsed.data);
