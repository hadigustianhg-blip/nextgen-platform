...
export async function GET() {
  ...
  if (!canAccessResource(session.roles, "USER_PROFILE", "READ")) return NextResponse.json({ success: false, error: { code: "FORBIDDEN" } }, { status: 403, headers: noStore });
  try {
    return NextResponse.json({ success: true, data: await getOwnProfile(session) }, { headers: noStore });
  } catch (error) {
...
export async function PATCH(request: Request) {
  ...
  const parsed = ownProfileUpdateSchema.safeParse(await request.json().catch(() => null));
  ...
  try {
    return NextResponse.json({ success: true, data: await updateOwnProfile(session, parsed.data.name) }, { headers: noStore });
  } catch (error) {
    return profileErrorResponse(error);
  }
}
