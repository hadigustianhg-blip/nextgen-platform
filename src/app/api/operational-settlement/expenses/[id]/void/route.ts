...
import { canMutateExpense, operationalScope, voidExpenseSchema, voidOperationalExpense } from "@/modules/operational-settlement";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const row = await voidOperationalExpense({ ...scope, actorId: session.userId }, (await context.params).id, parsed.data);
    return row ? NextResponse.json({ data: row }) : NextResponse.json({ error: { code: "NOT_FOUND" } }, { status: 404 });
  } catch (error) {
    const code = error instanceof Error ? error.message : "VOID_FAILED";
    return NextResponse.json({ error: { code, message: "Pengeluaran tidak dapat dibatalkan." } }, { status: 409 });
  }
}
