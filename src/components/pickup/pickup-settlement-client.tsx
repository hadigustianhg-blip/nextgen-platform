...
export function PickupSettlementClient({ outletCode }: { outletCode: string }) {
  ...
  useEffect(() => {
    queueMicrotask(async () => {
      try {
        const response = await fetch("/api/pickup/transfer-accounts", { cache: "no-store" });
  ...
  async function syncPickup() {
    ...
    try {
      ...
      setNotice({ tone: "success", text: `Sync selesai: ${body.data.created} baru, ${body.data.updated} diperbarui.` });
      setPage(1);
      await loadData();
    } catch (error) {
      setNotice({ tone: "error", text: error instanceof Error ? error.message : "Sinkronisasi gagal." });
    } finally {
      setSyncing(false);
    }
  }
  ...
  async function openAdjustment(row: SettlementRow) {
    ...
    try {
      const [detailResponse, accountsResponse] = await Promise.all([
        fetch(`/api/pickup/settlements/${row.id}`, { cache: "no-store" }),
    ...
    try {
      const response = await fetch(`/api/pickup/settlements/${selected.id}/adjust`, {
        ...
        body: JSON.stringify({
          ...
          transferAccountId:
            adjustmentStatus === "SUDAH_BAYAR" && adjustmentMethod === "TRANSFER"
              ? accountId || null
              : null,
          note: note || null,
        }),
      ...
      if (!response.ok) throw new Error(body.error?.message);
      setSelected(null);
      setNotice({ tone: "success", text: `Penyesuaian ${body.data.waybillNo} berhasil disimpan.` });
      await loadData();
    } catch (error) {
      setNotice({ tone: "error", text: error instanceof Error ? error.message : "Penyesuaian gagal." });
    } finally {
      setSaving(false);
    }
  }
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? <tr><td colSpan={10} className="py-16 text-center text-slate-500"><LoaderCircle className="mx-auto mb-2 animate-spin" />Memuat data…</td></tr> : rows.length === 0 ? <tr><td colSpan={10} className="py-16 text-center text-slate-500">Belum ada pickup Tunai.</td></tr> : rows.map((row) => (
                <tr key={row.id} className="hover:bg-blue-50/30">
                  ...
                  <td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-bold ${row.paymentStatus === "SUDAH_BAYAR" ? "bg-emerald-50 text-emerald-700" : row.paymentStatus === "LEBIH_BAYAR" ? "bg-violet-50 text-violet-700" : "bg-amber-50 text-amber-700"}`}>{statusLabels[row.paymentStatus]}</span></td>
                  ...
                  <td className="max-w-52 truncate px-4 py-3" title={row.note ?? ""}>{row.note ?? "—"}</td>
                  <td className="px-4 py-3"><button onClick={() => void openAdjustment(row)} className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-bold text-blue-700">Penyesuaian</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-label="Penyesuaian Pickup">
          <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
              <div><h2 className="text-xl font-extrabold text-slate-900">Penyesuaian Pickup</h2><p className="mt-1 text-xs text-slate-500">Revision tersimpan tanpa menghapus histori.</p></div>
              <button onClick={closeModal} disabled={saving} aria-label="Tutup modal" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><X size={20} /></button>
            </div>
            <div className="grid gap-3 bg-slate-50 p-6 sm:grid-cols-2 lg:grid-cols-3">
              {[["Tanggal", formatDateTime(selected.operationalDate)], ["Waybill", selected.waybillNo], ["Staff", selected.staff ?? "—"], ["Pengirim", selected.sender ?? "—"], ["Ongkir", formatMoney(selected.freightAmount)], ["Total Diterima saat ini", formatMoney(selected.totalPaid)]].map(([label, value]) => <div key={label} className="rounded-xl bg-white p-3"><p className="text-[10px] font-bold uppercase text-slate-400">{label}</p><p className="mt-1 text-sm font-bold text-slate-800">{value}</p></div>)}
            </div>
            <div className="grid gap-4 p-6 sm:grid-cols-2">
              ...
              <label className="text-sm font-semibold text-slate-700">Status
                <select value={adjustmentStatus} onChange={(event) => { const status = event.target.value as "BELUM_BAYAR" | "SUDAH_BAYAR"; setAdjustmentStatus(status); if (status === "BELUM_BAYAR") { setAdjustmentMethod(""); setAccountId(""); } }} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 font-normal">
              ...
              <label className="text-sm font-semibold text-slate-700">Metode Bayar
                <select disabled={adjustmentStatus === "BELUM_BAYAR"} value={adjustmentMethod} onChange={(event) => { const method = event.target.value as "" | "TUNAI" | "TRANSFER"; setAdjustmentMethod(method); if (method !== "TRANSFER") setAccountId(""); }} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 font-normal disabled:bg-slate-100">
              ...
              <label className="text-sm font-semibold text-slate-700">Rekening Transfer
                <select disabled={adjustmentStatus !== "SUDAH_BAYAR" || adjustmentMethod !== "TRANSFER"} value={accountId} onChange={(event) => setAccountId(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 font-normal disabled:bg-slate-100">
