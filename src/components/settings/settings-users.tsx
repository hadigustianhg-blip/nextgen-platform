...
import { KeyRound, Pencil, Plus, Search, UserCheck, UserX, X } from "lucide-react";
...
type SettingsUserResponse = { data?: unknown; error?: { code?: string } };
...
export function SettingsUsers({ data, outletCode, reload }: { data: SettingsUserRow[]; outletCode: string; reload: () => Promise<void> }) {
  ...
  async function openCreate() {
    setError(""); setMessage(""); setForm(emptyForm); setEditing("new");
  ...
  async function saveUser() {
    ...
    try {
      ...
      setEditing(null); setMessage(isCreate ? "User berhasil ditambahkan." : "User berhasil diperbarui."); await reload();
  ...
  const validUserForm = form.name.trim().length >= 2 && form.email.includes("@") && (form.userType === "ADMIN_WEB" ? Boolean(form.roleCode) : Boolean(form.salaryEmployeeId)) && (editing !== "new" || (form.password.length >= 10 && form.password === form.confirmPassword));
  ...
  return <div className="space-y-5">
    ...
    <SettingsCard title="User & Hak Akses">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-3">
        <select className={inputClass} value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="ALL">Semua Tipe</option><option value="ADMIN_WEB">Admin Web</option><option value="TEAM_PWA">Team / Kurir PWA</option></select>
      ...
      <div className="mt-4 overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b text-left text-slate-500">{["Nama", "Email / Username", "Tipe User", "Role", "Status", "Login Terakhir", "Aksi"].map((item) => <th key={item} className="px-3 py-3">{item}</th>)}</tr></thead><tbody>{filtered.map((user) => <tr key={user.id} className="border-b border-slate-100"><td className="px-3 py-3 font-semibold text-slate-800">{user.name}</td><td className="px-3">{user.email}</td><td className="px-3">{typeOf(user) === "TEAM_PWA" ? "Team / Kurir PWA" : "Admin Web"}</td><td className="px-3"><Badge tone={roleOf(user) === "TEAM" ? "blue" : "slate"}>{roleOf(user)}</Badge></td><td className="px-3"><Badge tone={user.status === "ACTIVE" ? "green" : "red"}>{user.status === "ACTIVE" ? "Aktif" : "Nonaktif"}</Badge></td><td className="px-3">{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString("id-ID") : "—"}</td><td className="whitespace-nowrap px-3"><button className="p-2 text-blue-700" title="Edit" onClick={() => void openEdit(user)}><Pencil size={16}/></button><button className="p-2 text-slate-600" title="Reset Password" onClick={() => { setPasswordFor(user); setForm(emptyForm); setError(""); }}><KeyRound size={16}/></button><button className={`p-2 ${user.status === "ACTIVE" ? "text-red-600" : "text-emerald-700"}`} title={user.status === "ACTIVE" ? "Nonaktifkan" : "Aktifkan"} onClick={() => setStatusFor(user)}>{user.status === "ACTIVE" ? <UserX size={16}/> : <UserCheck size={16}/>}</button></td></tr>)}</tbody></table>{filtered.length === 0 && <p className="p-6 text-center text-sm text-slate-500">Tidak ada user yang sesuai filter.</p>}</div>
    </SettingsCard>

    {editing && <Modal title={editing === "new" ? "Tambah User" : "Edit User"} onClose={() => setEditing(null)}><div className="grid gap-4 sm:grid-cols-2"><Field label="Nama Lengkap"><input className={inputClass} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })}/></Field><Field label="Email / Username"><input type="email" className={inputClass} value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })}/></Field><Field label="Tipe User"><select className={inputClass} value={form.userType} onChange={(event) => setForm({ ...form, userType: event.target.value as UserType, roleCode: event.target.value === "TEAM_PWA" ? "TEAM" : "ADMIN", salaryEmployeeId: "" })}><option value="ADMIN_WEB">Admin Web</option><option value="TEAM_PWA">Team / Kurir PWA</option></select></Field><Field label="Status"><select className={inputClass} value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as UserForm["status"] })}><option value="ACTIVE">Aktif</option><option value="SUSPENDED">Nonaktif</option></select></Field><Field label="Outlet"><input className={inputClass} value={outletCode || "Outlet session aktif"} disabled/></Field>{form.userType === "ADMIN_WEB" ? <Field label="Role"><select className={inputClass} value={form.roleCode} onChange={(event) => setForm({ ...form, roleCode: event.target.value })}>{adminRoles.map((role) => <option key={role}>{role}</option>)}</select></Field> : <><Field label="Role"><input className={inputClass} value="TEAM" disabled/></Field><Field label="SalaryEmployee terkait"><select className={inputClass} value={form.salaryEmployeeId} onChange={(event) => setForm({ ...form, salaryEmployeeId: event.target.value })}><option value="">Pilih Team/Kurir aktif</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name} · {employee.division}</option>)}</select></Field><Field label="Nomor WhatsApp"><input className={inputClass} value={selectedEmployee?.whatsapp ?? "—"} disabled/></Field></>}{editing === "new" && <><Field label="Password"><input type="password" autoComplete="new-password" className={inputClass} value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })}/></Field><Field label="Konfirmasi Password"><input type="password" autoComplete="new-password" className={inputClass} value={form.confirmPassword} onChange={(event) => setForm({ ...form, confirmPassword: event.target.value })}/></Field></>}</div><div className="mt-5 flex justify-end gap-2"><button className={`${buttonClass} bg-slate-100 text-slate-700`} onClick={() => setEditing(null)}>Batal</button><button className={buttonClass} disabled={!validUserForm || saving} onClick={() => void saveUser()}>{saving ? "Menyimpan..." : "Simpan User"}</button></div></Modal>}
