...
type Closing = { status: "OPEN" | "CLOSED"; physicalCash: string; cashVariance: string | null; varianceStatus: "NOT_SET" | "MATCH" | "SHORT" | "OVER"; closedAt: string | null; reopenReason: string | null };
...
export function OperationalSettlementClient({ outletCode, canAdmin }: { outletCode: string; canAdmin: boolean }) {
  ...
  const [closing, setClosing] = useState<Closing>({ status: "OPEN", physicalCash: "0", cashVariance: null, varianceStatus: "NOT_SET", closedAt: null, reopenReason: null });
  ...
  async function closeDay() {
    ...
    try {
      const response = await fetch("/api/operational-settlement/closing", {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ requestKey: crypto.randomUUID(), operationalDate, physicalCash: digits(physicalCash) }),
      });
      if (!response.ok) throw new Error();
      setNotice({ tone: "success", text: "Operasional harian berhasil ditutup." }); await load();
    } catch { setNotice({ tone: "error", text: "Operasional belum dapat ditutup." }); }
    finally { setSaving(false); }
  }
  ...
  const variance = BigInt(digits(physicalCash)) - BigInt(summary.cashAvailable.split(".")[0] || "0");
  ...
  return <div className="space-y-6">
    ...
    ].map(([label, value]) => <article key={label} className="rounded-2xl border bg-white p-5 shadow-sm"><p className="text-xs font-bold uppercase text-slate-500">{label}</p><p className="mt-3 text-2xl font-extrabold">{money(value)}</p></article>)}</div>
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-bold">Closing Harian</h2><p className="mt-1 text-sm text-slate-500">Cash tersedia: {money(summary.cashAvailable)}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${closing.status === "CLOSED" ? "bg-slate-900 text-white" : "bg-emerald-100 text-emerald-700"}`}>{closing.status === "CLOSED" ? "Closed" : "Open"}</span></div><div className="mt-5 grid gap-4 md:grid-cols-3"><label className="text-sm font-semibold">Physical Cash<input disabled={closing.status === "CLOSED"} inputMode="numeric" value={physicalCash} onChange={(event) => setPhysicalCash(digits(event.target.value))} className="mt-1 h-11 w-full rounded-xl border px-3 disabled:bg-slate-50" /></label><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Cash Variance</p><p className="mt-1 text-lg font-bold">{money(String(variance < 0n ? -variance : variance))}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Status</p><p className="mt-1 text-lg font-bold">{varianceLabel}</p></div></div>{closing.status === "OPEN" ? <button disabled={!operationalDate || saving || !canAdmin} onClick={() => void closeDay()} className="mt-4 rounded-xl bg-slate-900 px-5 py-2.5 font-bold text-white disabled:opacity-40">Tutup Operasional</button> : canAdmin && <div className="mt-4 flex flex-col gap-3 sm:flex-row"><input value={reopenReason} onChange={(event) => setReopenReason(event.target.value)} placeholder="Alasan buka kembali" className="h-11 flex-1 rounded-xl border px-3" /><button disabled={saving || reopenReason.trim().length < 3} onClick={() => void reopenDay()} className="rounded-xl border border-amber-300 px-5 py-2.5 font-bold text-amber-700 disabled:opacity-40">Buka Kembali</button></div>}</section>
