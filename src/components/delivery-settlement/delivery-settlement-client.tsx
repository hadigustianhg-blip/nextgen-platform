...
export function DeliverySettlementClient({ outletCode }: { outletCode: string }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <MetricCard label={<span className="flex items-center gap-2"><Banknote size={17} />COD Tunai</span>} value={money(summary.totalCod)} />
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="overflow-x-auto"><table className="min-w-[1650px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr>{["Waktu Diperbarui","Tanggal","Nama Kurir","Pickup Tunai","DFOD","COD Tunai","COD QRIS","Total Setoran","Bayar Tunai","Total Transfer","Total Diterima","Belum Bayar","Status","Aksi"].map((item) => <th key={item} className="px-4 py-3">{item}</th>)}</tr></thead>
      <tbody className="divide-y divide-slate-100">{loading ? <tr><td colSpan={14} className="py-14 text-center text-slate-500"><LoaderCircle className="mx-auto mb-2 animate-spin" />Memuat data…</td></tr> : rows.length === 0 ? <tr><td colSpan={14} className="py-14 text-center text-slate-500"><Truck className="mx-auto mb-2" />Belum ada Delivery Settlement.</td></tr> : rows.map((row) => <tr key={row.id} className="hover:bg-slate-50"><td className="px-4 py-3">{dateTime(row.updatedAt)}</td><td className="px-4 py-3">{date(row.operationalDate)}</td><td className="px-4 py-3 font-semibold">{row.courierName}</td>{[row.pickupCashAmount,row.dfodAmount,row.codCashAmount,row.codQrisAmount,row.totalSettlement,row.cashPaidAmount,row.transferPaidAmount,row.totalReceived,row.outstandingAmount].map((amount, index) => <td key={index} className="px-4 py-3 tabular-nums">{money(amount)}</td>)}<td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${row.paymentStatus === "CLEAR" ? "bg-emerald-100 text-emerald-700" : row.paymentStatus === "OVERPAID" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-800"}`}>{row.paymentStatus === "CLEAR" ? "Clear" : row.paymentStatus === "OVERPAID" ? "Lebih Bayar" : "Belum Clear"}</span></td><td className="px-4 py-3"><button onClick={() => open(row)} className="font-semibold text-blue-600 hover:text-blue-800">Penyesuaian</button></td></tr>)}</tbody></table></div>
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4"><div role="dialog" aria-modal="true" aria-label="Setor Kurir" className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
      ...
      <div className="grid gap-6 p-6 lg:grid-cols-2"><div className="space-y-5"><section className="rounded-2xl bg-slate-50 p-5"><h3 className="font-bold">Ringkasan Kurir</h3><dl className="mt-4 grid grid-cols-2 gap-4 text-sm">{[["Tanggal",date(selected.operationalDate)],["Nama Kurir",selected.courierName],["Pickup Tunai",money(selected.pickupCashAmount)],["DFOD",money(selected.dfodAmount)],["COD Tunai",money(selected.codCashAmount)],["COD QRIS",money(selected.codQrisAmount)]].map(([label,value]) => <div key={label}><dt className="text-slate-500">{label}</dt><dd className="mt-1 font-semibold">{value}</dd></div>)}</dl></section>
