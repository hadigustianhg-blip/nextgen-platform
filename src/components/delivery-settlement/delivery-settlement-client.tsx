...
export function DeliverySettlementClient({ outletCode }: { outletCode: string }) {
  ...
  async function save() {
    ...
    try {
      const response = await fetch(`/api/delivery-settlement/${selected.id}/adjustment`, {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({
          requestKey: crypto.randomUUID(), cashAmount: rupiahDigits(cash),
          transfers: buildTransferPayload(transfers),
          note: note || null,
        }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error?.message);
      setSelected(null); setNotice({ tone: "success", text: "Setoran kurir berhasil disimpan." }); await load();
  ...
  return <div className="space-y-6">
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4"><div role="dialog" aria-modal="true" aria-label="Setor Kurir" className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
      ...
      <section><h3 className="font-bold">Form Pembayaran</h3><label className="mt-4 block text-sm font-medium">Bayar Tunai<input inputMode="numeric" value={cash} onChange={(event) => setCash(rupiahDigits(event.target.value))} className="mt-1 w-full rounded-xl border px-3 py-2.5" /></label>
      <div className="mt-5"><h4 className="text-sm font-bold text-slate-800">Transfer</h4><div className="mt-3 space-y-3">{transfers.map((value,index) => <div key={index} className="rounded-xl border border-slate-200 p-3 sm:flex sm:items-end sm:gap-3"><label className="block flex-1 text-sm font-medium">Transfer {index + 1}<input aria-label={`Transfer ${index + 1}`} inputMode="numeric" value={value} onChange={(event) => setTransfers((current) => current.map((item,i) => i === index ? rupiahDigits(event.target.value) : item))} className="mt-1 w-full rounded-xl border px-3 py-2.5" /></label><button type="button" aria-label={`Hapus Transfer ${index + 1}`} onClick={() => removeTransfer(index)} className="mt-2 min-h-11 w-full rounded-xl border border-red-200 px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 sm:mt-0 sm:w-auto">Hapus</button></div>)}</div>
      {transfers.length < 8 ? <button type="button" aria-label="Tambah Transfer" onClick={addTransfer} className="mt-3 min-h-11 rounded-xl border border-blue-200 px-4 py-2 text-sm font-semibold text-blue-600 hover:bg-blue-50">+ Tambah Transfer</button> : <p className="mt-3 text-xs font-medium text-slate-500">Maksimal 8 transfer</p>}</div>
      <label className="mt-4 block text-sm font-medium">Keterangan Belum Bayar<textarea maxLength={500} value={note} onChange={(event) => setNote(event.target.value)} className="mt-1 min-h-20 w-full rounded-xl border px-3 py-2.5" /></label></section></div>
