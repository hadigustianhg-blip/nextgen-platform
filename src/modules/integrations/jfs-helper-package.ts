...
const ALWAYS_FORBIDDEN_CONTENT = [
...
const PRODUCTION_HELPER_URL = "https://app.nextgen-platform.com/helper/pickup-adjustment";
const TARGETS = {
  development: { archiveName: "nextgen-jfs-helper-dev.zip", baseUrl: "https://dev.nextgen-platform.com", helperUrl: DEV_HELPER_URL },
  production: { archiveName: "nextgen-jfs-helper.zip", baseUrl: "https://app.nextgen-platform.com", helperUrl: PRODUCTION_HELPER_URL },
} as const;
...
export type JfsHelperPackageTarget = keyof typeof TARGETS;
...
function transformRuntimeFile(filename: string, content: Buffer, target: JfsHelperPackageTarget) {
  if (target === "development") return content;
  let text = content.toString("utf8").replaceAll(TARGETS.development.baseUrl, TARGETS.production.baseUrl);
  if (filename === "manifest.json") {
    const manifest = JSON.parse(text) as { name: string; description: string };
    manifest.name = manifest.name.replace("NEXTGEN DEV", "NEXTGEN");
    manifest.description = manifest.description.replace("NEXTGEN DEV", "NEXTGEN Production");
    text = `${JSON.stringify(manifest, null, 2)}\n`;
  } else if (filename === "README.md") {
    text = text
      .replace("Versi ini hanya untuk NEXTGEN DEV.", "Versi ini untuk NEXTGEN Production.")
      .replaceAll("NEXTGEN DEV", "NEXTGEN Production");
  }
  return Buffer.from(text, "utf8");
}

export async function buildJfsHelperPackage(options: { sourceRoot?: string; target: JfsHelperPackageTarget }): Promise<JfsHelperPackage> {
  const sourceRoot = options.sourceRoot ?? path.join(/*turbopackIgnore: true*/ process.cwd(), "extensions", "nextgen-jfs-helper");
  ...
  const entries = await Promise.all(JFS_HELPER_RUNTIME_FILES.map(async (filename) => {
    ...
    const content = transformRuntimeFile(filename, await readFile(path.join(sourceRoot, filename)), options.target);
    return { name: `${JFS_HELPER_ARCHIVE_DIRECTORY}/${filename}`, content };
  }));
  const combined = entries.map(({ content }) => content.toString("utf8")).join("\n");
  for (const check of ALWAYS_FORBIDDEN_CONTENT) {
    if (check.pattern.test(combined)) throw new Error(`Extension package contains forbidden ${check.label}.`);
  }
  const forbiddenTarget = options.target === "development" ? TARGETS.production.baseUrl : TARGETS.development.baseUrl;
  if (combined.includes(forbiddenTarget)) throw new Error(`Extension package contains the wrong ${options.target} helper target.`);
  if (!combined.includes(target.helperUrl)) throw new Error(`Extension package does not contain the canonical ${options.target} helper URL.`);
