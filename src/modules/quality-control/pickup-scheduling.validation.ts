...
export const pickupSchedulingQuerySchema = z.object({
  ...
  sourcePlatform: z.string().trim().max(100).default(""),
  orderStatus: z.string().trim().max(100).default(""),
  sendName: z.string().trim().max(100).default(""),
  pickupNetwork: z.string().trim().max(100).default(""),
  pickupStaff: z.string().trim().max(100).default(""),
  assignment: z.enum(["ALL", "ASSIGNED", "UNASSIGNED"]).default("ALL"),
  pickupFailure: z.enum(["ALL", "FAILED", "NOT_FAILED"]).default("ALL"),
  senderCity: z.string().trim().max(100).default(""),
  senderArea: z.string().trim().max(100).default(""),
