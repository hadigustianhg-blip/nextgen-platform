...
await writeFile(path.join(outputDirectory, JFS_HELPER_ARCHIVE_NAME), result.archive);
...
if (entries.length !== result.files.length || entries.some((entry, index) => entry !== result.files[index])) {
  throw new Error("Generated extension ZIP failed entry validation.");
}
console.info(`JFS_HELPER_PACKAGE_READY: ${JFS_HELPER_ARCHIVE_NAME} v${result.version}`);
